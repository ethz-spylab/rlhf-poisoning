...
class RLTrainer(TrainerBase):  # pylint: disable=too-many-instance-attributes
        ...
        if self.args.need_eval:
            if (
                ...
                (
                    prompt_only_dataset,
                    eval_dataset,
                ) = prompt_only_dataset.split_train_test(
    ...
    def rollout(self, prompt_only_batch: PromptOnlyBatch) -> list[dict[str, Any]]:
        ...
        return [
            self.post_rollout(
                ...
                output_multiplier=prompt_only_batch["output_multiplier"]
                if "output_multiplier" in prompt_only_batch
                else None,
            )
    ...
    def train(self) -> None:
        ...
        num_ptx_replicas = (
            num_prompt_only_batches + num_ptx_batches - 1
        ) // num_ptx_batches
                ...
                for _ in range(self.args.update_iters):
                    for rl_batch, ptx_batch in zip(rl_batches, ptx_batches):
                        ...
                        if global_step % self.args.save_interval == 0:
                            self.logger.print(
                                f"Saving checkpoint at step {global_step} ..."
                            )
                            self.actor_model.save_checkpoint(
                                self.args.output_dir, tag=global_step
                            )
    ...
    def eval(self) -> dict[str, Any]:
        ...
        for batch in eval_dataloader:
            ...
            for key, values in self.eval_step(
                seq, attention_mask, output_multiplier=batch["output_multiplier"]
            ).items():
        ...
        if is_main_process():
            ...
            concatenated_scores = {
                key: torch.cat(value, dim=0).to(torch.float32)
