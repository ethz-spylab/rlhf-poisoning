...
class SupervisedDataset(TokenizedDataset):
    def preprocess(self, raw_sample: RawSample) -> SupervisedSample:
        ...
        if (
            raw_sample.get("input") is not None
            and raw_sample.get("dialogue") is not None
        ):
        ...
        dialogue = (
            dialogue + [raw_sample["answer"]] if "answer" in raw_sample else dialogue
        )  # If the answer is separate, include in dialogue
        dialogue[-1] += self.tokenizer.eos_token
